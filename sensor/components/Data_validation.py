...
class DataValidation:
    ...
    def drop_missing_value_column(self,df:pd.DataFrame,report_key_name:str)->Optional[pd.DataFrame]:
        ...
        try:
            ...
            logging.info(f"selecting column name which contains null abone to {thresold}")
    ...
    def is_required_columns_exist(self,base_df:pd.DataFrame,current_df:pd.DataFrame,report_key_name:str)->bool:
        try:
            ...
            logging.info(f"columns: current {(current_columns)} and  base col : {base_columns} ")
    ...
    def initiate_data_validation(self)->artifacts_entity.DataValidationArtifact:
        try:
            ...
            logging.info(f"drop 'null' values columns from base DataFrame {base_df.shape}")
            base_df =self.drop_missing_value_column(df=base_df,report_key_name="missing_values_within_base_dset")
            ...
            train_df =self.drop_missing_value_column(df=train_df,report_key_name="missing_values_within_train_dset")
            logging.info(f"drop 'null' values columns from train DataFrame {train_df.shape}")
            test_df =self.drop_missing_value_column(df=test_df,report_key_name="missing_values_within_test_dset")
            logging.info(f"drop 'null' values columns from test DataFrame {test_df.shape}")


            exclude_columns=["class"]

            base_df=convert_column_to_float(df=base_df,exclude_columns=exclude_columns)
            train_df=convert_column_to_float(df=train_df,exclude_columns=exclude_columns)
            test_df=convert_column_to_float(df=test_df,exclude_columns=exclude_columns)

            
            logging.info("checking for required column exist in train DataFrame")
            train_df_col_status =self.is_required_columns_exist(base_df=base_df,current_df=train_df,report_key_name="missing_columns_within_train_dset")
            logging.info("checking for required column exist in test DataFrame")
            test_df_col_status =self.is_required_columns_exist(base_df=base_df,current_df=test_df,report_key_name="missing_columns_within_test_dset")
            ...
            if train_df_col_status:
                ...
                self.data_drift(base_df=base_df,current_df=train_df,report_key_name="data_drift_within_train_dset")
            if test_df_col_status:
                logging.info("checking data drift in test DataFrame")
                self.data_drift(base_df=base_df,current_df=test_df,report_key_name="data_drift_values_within_train_dset")

            # Write the report in yaml format
            logging.info("Writing report in yaml file ")
            write_yml_file(file_path=self.data_validation_config.report_file_path,data=self.validation_error)
